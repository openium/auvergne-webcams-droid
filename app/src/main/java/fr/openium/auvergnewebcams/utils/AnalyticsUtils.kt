...
object AnalyticsUtils {
    ...
    fun getUserPropertiesWebcamQualityPreferences(context: Context): String {
        val preferencesManager = PreferenceManager.getDefaultSharedPreferences(context)
        return preferencesManager.getString(KEY_WEBCAM_QUALITY_ENABLED, "high") ?: "high"
    }
    ...
    private fun sendFirebaseLogEventFavorite(firebaseAnalytics: FirebaseAnalytics, webcamName: String, becameFavorite: Boolean) {
        val bundle = Bundle().apply {
            putString(
                FirebaseAnalytics.Param.CONTENT_TYPE, if (becameFavorite) {
    ...
    // --- Ajout/Suppression d'une webcam aux favoris ---
    private fun sendFirebaseLogEventFavoriteFromSection(
    ...
    private fun sendFabricLogEventSelectDetail(answersAnalytics: Answers, webcamName: String) {
        answersAnalytics.logContentView(
            ContentViewEvent()
    ...
    // --- Ajout/Suppression d'une webcam aux favoris ---
    private fun sendFabricLogEventFavorite(answersAnalytics: Answers, webcamName: String, becameFavorite: Boolean) {
        answersAnalytics.logContentView(
            ContentViewEvent()
    ...
    // --- Ajout/Suppression d'une webcam aux favoris ---
    private fun sendFabricLogEventFavoriteFromSection(answersAnalytics: Answers, webcamName: String, becameFavorite: Boolean) {
        answersAnalytics.logContentView(
            ContentViewEvent()
                .putContentName(KEY_SPECIAL_EVENTS_CONTENT_TYPE_FAVORITE)
                .putContentType(
    ...
    fun webcamDetailRefreshed(context: Context) {
        AnalyticsUtils.sendFirebaseLogEventSelectContent(
            FirebaseAnalytics.getInstance(context),
            AnalyticsUtils.KEY_BUTTON_WEBCAM_DETAIL_REFRESH
        )
    ...
    fun buttonSignalProblemClicked(context: Context) {
        AnalyticsUtils.sendFirebaseLogEventSelectContent(
            FirebaseAnalytics.getInstance(context),
            AnalyticsUtils.KEY_BUTTON_REPORT_WEBCAM_ERROR
        )
        AnalyticsUtils.sendFabricLogEventSelectContent(Answers.getInstance(), AnalyticsUtils.KEY_BUTTON_REPORT_WEBCAM_ERROR)
    }

    fun buttonFavoriteClicked(context: Context, webcamTitle: String, becameFavorite: Boolean) {
        AnalyticsUtils.sendFirebaseLogEventFavorite(FirebaseAnalytics.getInstance(context), webcamTitle, becameFavorite)
        AnalyticsUtils.sendFabricLogEventFavorite(Answers.getInstance(), webcamTitle, becameFavorite)
    }
    ...
    fun buttonFavoriteFromSectionClicked(context: Context, webcamTitle: String, becameFavorite: Boolean) {
        AnalyticsUtils.sendFirebaseLogEventFavoriteFromSection(FirebaseAnalytics.getInstance(context), webcamTitle, becameFavorite)
        AnalyticsUtils.sendFabricLogEventFavoriteFromSection(Answers.getInstance(), webcamTitle, becameFavorite)
    }
}